...
有音节连单字都查不到（只能拿拼音占位）的不出句子；英文词的位置看 `Engine::chinese_first`（配置 `[general] chinese_first`，缺省开）：
开着时整句先插、英文词紧随其后排第二（`hello` 先 荷兰咯 再 hello）；关着时整段是英文词或不像拼音带出的英文补全排在句子前面（`hello` 先英文再 荷兰咯）。
